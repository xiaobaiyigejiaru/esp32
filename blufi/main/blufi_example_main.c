...
static void example_wifi_connect(void)
{
    ...
    BLUFI_INFO("gl_sta_is_connecting%d\n", gl_sta_is_connecting); //
                               ...
                               int32_t event_id, void *event_data)
{
    ...
    case WIFI_EVENT_STA_START:
        ...
        example_wifi_connect(); // 1
        break;
    case WIFI_EVENT_STA_CONNECTED:
        BLUFI_INFO("WIFI_EVENT_STA_CONNECTED\n");
        BLUFI_INFO("gl_sta_connected=%d\n", gl_sta_connected);
    ...
    case WIFI_EVENT_STA_DISCONNECTED:
        ...
        if (gl_sta_is_connecting == false && gl_sta_connected == true)
        {
            BLUFI_INFO("Only first disconected\n");
            example_wifi_retry = 0;
            gl_sta_is_connecting = true;
            gl_sta_connected = false;
        }

        /* Only handle reconnection during connecting wifi 未连接，发起连接失败*/
        ...
        /* This is a workaround as ESP32 WiFi libs don't currently
           auto-reassociate. 用户自定义重连*/
        gl_sta_got_ip = false;
        memset(gl_sta_ssid, 0, 32);
        memset(gl_sta_bssid, 0, 6);
        gl_sta_ssid_len = 0;
        xEventGroupClearBits(wifi_event_group, CONNECTED_BIT);
        BLUFI_INFO("gl_sta_is_connecting=%d example_wifi_retry=%d\n", gl_sta_is_connecting, example_wifi_retry);
        break;
    ...
    {
        BLUFI_INFO("WIFI_EVENT_AP_STACONNECTED\n");
    ...
    {
        BLUFI_INFO("WIFI_EVENT_AP_STADISCONNECTED\n");
...
static esp_blufi_callbacks_t example_callbacks = {
    .event_cb = example_event_callback,                        // 事件处理
    .negotiate_data_handler = blufi_dh_negotiate_data_handler, // 数据协商，以此来生成共享密钥。
    .encrypt_func = blufi_aes_encrypt,                         // BLUFI（蓝牙 Wi-Fi 配置协议）相关的加密数据函数
    .decrypt_func = blufi_aes_decrypt,                         // BLUFI（蓝牙 Wi-Fi 配置协议）相关的解密数据函数
    .checksum_func = blufi_crc_checksum,                       // CRC
};
